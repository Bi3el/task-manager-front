...
import type { NewTask } from "@/interface/interface";
...
export function CreateTaskForm() {
    ...
    const { mutate: createTaskRequest, isPending, isError, error } = useMutation({
              ...
              <form className="space-y-4 ">
                ...
                <div className="w-full flex justify-end gap-3">
                    ...
                    onClick={form.handleSubmit(handleSubmit)}>
                        { isPending ? <Loader className="animate-spinl"/> : "Criar tarefa" }
                    </Button>
                </div>
                {isError && (
                  <p className="text-[0.8rem] font-medium text-red-500">Erro ao criar nova tarefa</p>
                )}
              </form>
            </Form>
        </div>
